# 1 "Serial.cpp"
# 1 "/home/impe/Arduino/dev/serial/Serial//"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 38 "/usr/include/c++/4.9/exception" 2 3
...
# 39 "/usr/include/c++/4.9/stdexcept" 2 3
...
public:
    void begin(const char* port, unsigned long baud);
...
Serial::~Serial()
{
 ...
 void Serial::begin(const char* port, unsigned long baud){

  fd = open (port, 02 | 0400 | 04010000);
  struct termios tty;
        memset (&tty, 0, sizeof tty);
        if (tcgetattr (fd, &tty) != 0)
        {
   throw SerialException("error %d from tcgetattr", (*__errno_location ()));
        ...
        {
                throw sprintf("error %d from tcgetattr", (*__errno_location ()));
 ...
 void Serial::end(){}
    ...
    size_t Serial::write(unsigned long n) { return write((uint8_t)n); }
    size_t Serial::write(long n) { return write((uint8_t)n); }
    size_t Serial::write(unsigned int n) { return write((uint8_t)n){}; }
    size_t Serial::write(int n) { return write((uint8_t)n){}; }
 size_t Serial::write(char* str){
  }
 size_t Serial::write(char* buf, size_t size){
  }

 const Serial::operator bool(){
  return status;
 }
